...
ram_write inst4(.in1(data_out_pe1),.in2(data_out_pe2),.in3(data_out_pe3) ,.enable(enable_out),.clk(clk),.ram_write_enable(ram_write_enable),. ram_addr(ram_write_addr),.ram_write_data(ram_write_data));
writeram inst5(.addr(ram_write_addr),.clk(clk),.enable_ram(ram_write_enable),.data_in(ram_write_data));
endmodule